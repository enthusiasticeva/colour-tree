...
class Node:
    ...
    * children() -> list(Node) : Returns the list of children in this node.
    ...
    def update_colour(self, colour: Colour) -> None:
        ...
        self.colour = colour
        # If there are no children, then the propagated colour will just be the colour
        self.propagated_colour = colour
        
        

        # Goes through and finds the strongest child, changing the prop colur if a stringer colour is found
        for child in self.children:
          if child.propagated_colour.cmp(self.propagated_colour) >= 1:
            #print(f"updating {self.propagated_colour.NAME} to {child.propagated_colour.NAME}")
    ...
    def add_child(self, child_node: 'Node') -> None:
        ...
        # Recalculates the propagated colour based on the child being added
        if child_node.propagated_colour.cmp(self.colour) >= 1:
            self.propagated_colour = child_node.propagated_colour
